...
var AblityList = [
  {
    ...
    //14
    name : "중력화살 (액티브)"
    detail : ["(쿨타임 5초) 자신이 포커스를 맞춘 위치에 화살이 3×3으로 빠른 속도로 떨어집니다. (범위 10블럭 이내)", "(쿨타임 5초) 자신이 포커스를 맞춘 위치에 화살이 3×3으로 빠른 속도로 떨어집니다. (범위 15블럭 이내)", "(쿨타임 5초) 자신이 포커스를 맞춘 위치에 화살이 5×5으로 빠른 속도로 떨어집니다. (범위 17블럭 이내)"]
  }
];
    ...
    if(cmd[1].equals("obs")) {
      ...
      for(var i in player) {
        if(cmd[2].equals(Player.getName(player[i]) {
          ...
          net.zhuoweizhang.mcpelauncher.ScriptManager.nativeSendChat(Player.getName(player[i]) + "님이 관전자 목록에 등록되었습니다.");
          player.splice(i,1);
        }
      }
